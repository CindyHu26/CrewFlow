...
export default function MyServicesList({ services }: MyServicesListProps) {
  ...
  const getFilteredServices = () => {
    ...
    if (!services || services.length === 0 || !currentUser) {
      console.log('No services to filter or no current user');
      return [];
    }
    ...
    const filtered = services.filter(service => {
      try {
        ...
        // 檢查是否為當前用戶的服務紀錄
        const isCurrentUserService = service.staff_name === currentUser || 
          (service.partner_names && service.partner_names.includes(currentUser));
        
        if (!isCurrentUserService) {
          console.log('Service not belongs to current user');
          return false;
        }
